...
/**
 * 客户端全局主配置对话框窗口（已移除 MUD 隔离配置）
 */
public class ClientConfigMenu extends AbsZmMudDialog {

    private ClientConfigView configView; // 核心配置面板（等宽字体、字号）
    private ClientConfigPresenter presenter; // 全局中枢 Presenter

    public ClientConfigMenu(Frame owner, String title) {
        super(owner, title);
    ...
    protected JPanel getContentPanelUi() {
        // 1. 实例化原子 UI 组件
        configView = new ClientConfigView();
        ...
        this.presenter = new ClientConfigPresenter(configView);
