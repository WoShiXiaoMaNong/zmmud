...
@Service
public class IacInbMsgReader implements InbMessageReader{
    ...
    public InbMessage readInbMessage(int firstByte,MudClient client) {
        ...
        if(IACConfirmProcessor.NON_OPTION_COMMANDS.contains(cmd)) {
            logger.info("Received IAC command from server: " + IACConfirmProcessor.CMD_NAME_MAP.getOrDefault(cmd, "UNKNOWN"));
        ...
        if(cmd == IACConfirmProcessor.CMD_SB) {
            ...
            // For simplicity, we will just read until we encounter IAC SE. In a real implementation, you would want to handle this more robustly.
        ...
        // Currently we only handle option negotiation commands, other IAC commands are treated as normal messages. Future we can add specific handling for other IAC commands if needed.
